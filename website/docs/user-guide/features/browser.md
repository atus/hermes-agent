...
**Session-owned tabs:** each actual Hermes session owns its own set of tabs, including when `session` is omitted. `session=<name>` selects a lane *within that owner*: the same alias in two chats or subagents does not share a daemon. Each lane has its own IPC/state and, on cloud backends, its own browser. Keep aliases stable per worker, not per URL. Compression continuations retain their original owner; real branches, subagents, and separate cron executions remain independent.

Use `goto_url(url)` for ordinary navigation, **including the first call**: Hermes automatically creates an owned tab instead of adopting your existing tab. Use `new_tab(url)` only to retain another page, such as a comparison. `list_tabs()` lists the lane's owned tabs, `current_tab()` returns the selected tab, `switch_tab(target_id)` selects an owned tab, and `close_tab(target_id)` closes one (omit the argument for the current tab). If the current tab disappears, the next navigation recovers an owned tab without selecting another session's page.

Tabs survive ordinary turns and stop/interruption of a turn. When the owner actually ends (for example, reset/replacement, exit, or completed worker teardown), cleanup closes its exact recorded tabs, opener-linked popups, and existing lane daemons. The native harness may keep one dedicated bootstrap blank tab per lane in addition to managed tabs; its graceful shutdown closes that tab. Cleanup waits for shutdown completion rather than treating its acknowledgment as completion. User tabs and other owners' tabs are left alone.

Cleanup is bounded and best-effort: failures retain ownership records and log a warning rather than claim success. Queued calls cannot silently reopen a closed generation; local in-flight CLI processes are canceled and drained. If execution is held by another process, cleanup may remain pending until that process releases the lane. **If the daemon is offline while the browser remains alive, recovery through IPC is unavailable:** this adapter does not start a new daemon or infer ownership from a browser-wide tab list. Records remain for retry/manual investigation; leaked tabs are not falsely reported as closed.

This is lifecycle isolation, not a sandbox for arbitrary Python or CDP. Advanced explicit CDP target/session access remains available for iframe/OOPIF workflows. Tab helper switching/closing stays scoped; raw target creation is recorded, and browser-wide teardown is rejected by the adapter. Code that bypasses the helper adapter is outside these guarantees.

For a manual end-to-end check, run `python scripts/check_browser_sessions.py --browser-use /path/to/browser-use` (optionally `--chromium /path/to/chromium`). It uses a disposable headless browser/profile, verifies 11 lifecycle checks, and checks that cleanup returns to its original unowned-tab baseline; it does not attach to your normal browser.
